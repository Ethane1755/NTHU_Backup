...
class Records:
    ...
    def find(self, inp, categories):
        ...
        if not found_records:  # Only check categories if no records have been found for 'expense' or 'income'
            valid_categories = [cat for cat in categories.find_subcategories(inp) if inp == cat]
            ...
            if valid_categories:
                ...
                found_records = [
                    ...
                    (inp in categories.find_subcategories(record.category))  # Check if inp is part of any nested category
                ]
...
class Categories:
    ...
    def is_category_valid(self, category):
        ...
        return category in self.find_subcategories(category)
            ...
            if isinstance(categories, list):  # Recursive case: list of categories
                for index, child in enumerate(categories):
                    ...
                    if child == category and index + 1 < len(categories) and isinstance(categories[index + 1], list):
                        # When category is found, recursively search for subcategories
                        yield from find_subcategories_gen(category, categories[index + 1], True)
            else:  # Base case: single category
                if found or categories == category:
                    yield categories

        # Return a list generated by the recursive generator
